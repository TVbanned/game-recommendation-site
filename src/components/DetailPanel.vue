...
import { sanitizeHtml } from '../utils/frontend'
...
const carouselImages = computed(() => { const game = props.game; if (!game) return []; const images = Array.isArray(game.screenshots) && game.screenshots.length ? game.screenshots : (game.screenshot ? [game.screenshot] : [game.cover]); return images.filter(Boolean) })
